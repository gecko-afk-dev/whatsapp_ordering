...
class EmailService:
    ...
    async def send_staff_invite_email(email: str, role: str, setup_token: str):
        """Mock function to send a staff invite email with role context."""
        setup_link = f"http://localhost:8000/static/index.html?setup_token={setup_token}"
        role_display = role.replace("_", " ").title()
        logger.info(
            f"\n{'='*50}\n"
            f"MOCK STAFF INVITE SENT TO: {email}\n"
            f"Subject: You've been invited to GEQO as {role_display}\n"
            f"Body: Click here to activate your {role_display} account: {setup_link}\n"
            f"{'='*50}\n"
        )
